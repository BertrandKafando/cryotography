package es.kf.signapp.external.validation;
...
import es.kf.signapp.external.validation.CertInformationCollector.CertSignatureInformation;
...
public class CertInformationHelper
{
                                                         ...
                                                         CertSignatureInformation certInfo) throws IOException
    {
        ...
        while (objects.hasMoreElements())
        {
            ...
            {
                ASN1OctetString url = (ASN1OctetString) location.getLoadedObject();
                certInfo.setOcspUrl(new String(url.getOctets()));
            }
            else if (X509ObjectIdentifiers.id_ad_caIssuers.equals(oid))
            {
                ASN1OctetString uri = (ASN1OctetString) location.getLoadedObject();
    ...
    private static String extractCrlUrlFromSequence(ASN1Sequence sequence)
    {
        ...
        taggedObject = (ASN1TaggedObject) taggedObject.getLoadedObject();
        if (taggedObject.getLoadedObject() instanceof ASN1TaggedObject)
        {
            taggedObject = (ASN1TaggedObject) taggedObject.getLoadedObject();
        }
        else if (taggedObject.getLoadedObject() instanceof ASN1Sequence)
        {
            // multiple URLs (we take the first)
            ASN1Sequence seq = (ASN1Sequence) taggedObject.getLoadedObject();
        ...
        if (taggedObject.getLoadedObject() instanceof ASN1OctetString)
        {
            ASN1OctetString uri = (ASN1OctetString) taggedObject.getLoadedObject();
